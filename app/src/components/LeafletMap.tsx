...
const buildHTML = (
  ...
  const zoom =
    initialRegion.latitudeDelta < 0.02
      ? 15
  ...
  const polylineCoords =
    history.length > 1
      ? `L.polyline([${history
          ...
          .join(
            ",",
          )}], {color:"#475569",weight:2,dashArray:"6,4",opacity:0.7}).addTo(map);`
      : "";
...
const LeafletMap = forwardRef<LeafletMapRef, LeafletMapProps>(
  function LeafletMap({ history, initialRegion, style }, ref) {
    const webViewRef = useRef<WebView>(null);

    useImperativeHandle(ref, () => ({
      animateTo: (latitude: number, longitude: number, zoom = 15) => {
